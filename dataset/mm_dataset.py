...
class MMDataset(Dataset):
    def __init__(self, data_path, mode, modal, fps, num_frames, len_feature, sampling, seed=-1, supervision='weak'):
        ...
        human_path = os.path.join(data_path, 'human_save_{}.json'.format(self.mode))
        with open(human_path, 'r') as f:
            self.human = json.load(f)
    ...
    def __getitem__(self, index):
        ...
        label, label_frames = self.get_label(index, vid_num_frame, sample_idx)

        sum_label = sum(label)
        if sum_label == 0:
            print("No label for this video")
            print(self.anno[index])
            import pdb; pdb.set_trace()

        # Get human information
        vid_name = self.anno[index]['video_url_1'].split('/')[-1].split('-')[0]
        human_info = torch.Tensor(self.human[vid_name])

        return data_video, data_audio, label, label_frames, vid_num_frame, human_info
