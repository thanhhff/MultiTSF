...
def inference(net, config, test_loader, model_file=None):
    ...
    with torch.no_grad():
        ...
        for _data, _label, _, vid_name, vid_num_seg in test_loader:
            ...
            # FORWARD PASS
            cas, (action_flow, action_rgb) = net(_data, is_training=False)
            ...
            if len(pred) != 0:
                ...
                for t in range(len(config.act_thresh)):
                    ...
                    proposals = misc_utils.get_proposal_oic(seg_list, cas_pred.copy(), score_supp[0, :].cpu().data.numpy(),
                                                            pred, config.scale, vid_num_seg[0].cpu().item(), config.fps,
        ...
        mean_ap, _ = evaluate(config.gt_path, json_path, None, tiou_thresholds=np.linspace(0.1, 0.9, 9), plot=False,
